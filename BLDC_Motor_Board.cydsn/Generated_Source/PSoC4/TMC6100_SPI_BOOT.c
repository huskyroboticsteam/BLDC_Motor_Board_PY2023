...
/*******************************************************************************
* Function Name: TMC6100_SpiCyBtldrCommStart
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: TMC6100_SpiCyBtldrCommStop
****************************************************************************//**
*
...
/*******************************************************************************
* Function Name: TMC6100_SpiCyBtldrCommReset
****************************************************************************//**
*
...
void TMC6100_SpiCyBtldrCommReset(void)
{
    ...
    TMC6100_SpiUartClearRxBuffer();
    TMC6100_SpiUartClearTxBuffer();
}


/*******************************************************************************
* Function Name: TMC6100_SpiCyBtldrCommRead
****************************************************************************//**
*
...
cystatus TMC6100_SpiCyBtldrCommRead(uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        {
            /* Check packet start */
            if (0u != TMC6100_SpiUartGetRxBufferSize())
            {
                /* Wait for packet end */
                do
                {
                    byteCount = TMC6100_SpiUartGetRxBufferSize();
                    CyDelayUs(TMC6100_SPI_BYTE_TO_BYTE);
                }
                while (byteCount != TMC6100_SpiUartGetRxBufferSize());

                /* Disable data reception into RX FIFO */
                TMC6100_RX_FIFO_CTRL_REG |= TMC6100_RX_FIFO_CTRL_FREEZE;

                byteCount = TMC6100_BYTES_TO_COPY(byteCount, size);
        ...
        {
            pData[i] = (uint8) TMC6100_SpiUartReadRxData();
        }
    }
...
/*******************************************************************************
* Function Name: TMC6100_SpiCyBtldrCommWrite
****************************************************************************//**
*
*  Allows the caller to read data from the bootloader host (the host writes the
*  data). The function handles polling to allow a block of data to be completely
...
cystatus TMC6100_SpiCyBtldrCommWrite(const uint8 pData[], uint16 size, uint16 * count, uint8 timeOut)
{
    ...
    if ((NULL != pData) && (size > 0u))
    {
        ...
        TMC6100_SpiUartPutArray(pData, (uint32) size);

        /* Wait with timeout 1mS for packet end */
        do
        {
            /* Check for packet end */
            if ((0u == TMC6100_SpiUartGetTxBufferSize()) &&
                (0u == (TMC6100_SPI_STATUS_REG & TMC6100_SPI_STATUS_BUS_BUSY)) &&
                (0u == (TMC6100_TX_FIFO_STATUS_REG & TMC6100_TX_FIFO_SR_VALID)))
            {
                *count = size;
                status = CYRET_SUCCESS;
        ...
        TMC6100_SpiUartClearRxBuffer();
        TMC6100_RX_FIFO_CTRL_REG &= (uint32) ~TMC6100_RX_FIFO_CTRL_FREEZE;
    }
